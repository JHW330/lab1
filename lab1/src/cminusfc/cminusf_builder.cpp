...
Value* CminusfBuilder::visit(ASTVarDeclaration &node) {
    ...
    if (scope.in_global()) {
        ...
        if (node.num != nullptr) {
            ...
            alloc = GlobalVariable::create(node.id, module.get(), array_type, false, ConstantZero::get(var_type, module.get()));
        } else {
            // 标量
        ...
        if (node.num != nullptr) {
            // 数组
            auto array_type = ArrayType::get(var_type, node.num->i_val);
            alloc = builder->create_alloca(array_type);
        } else {
            // 标量
...
Value* CminusfBuilder::visit(ASTIterationStmt &node) {
    // 创建循环的基本块
    auto *condBB = BasicBlock::create(module.get(), "while.cond", context.func);
    auto *bodyBB = BasicBlock::create(module.get(), "while.body", context.func);
    auto *endBB = BasicBlock::create(module.get(), "while.end", context.func);
